...
| `Viewport` | `Viewport.resolve`, `Viewport.safe`, `Viewport.clamp`, `Viewport.clampAccessible` | Pure player/environment rectangle resolution; no lifecycle owner. `clampAccessible` preserves a reachable header and close control while allowing deliberate partial off-screen placement. |
