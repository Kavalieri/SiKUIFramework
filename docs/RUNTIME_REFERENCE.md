...
presentation operation. Ownership records lifecycle and stacking without
replacing the owner's native methods or installing a permanent pointer blocker.
Closing, saving, cancelling or Escape disposal restores the owner layer and focus.
...
suppressed after instantiation. Focus uses the palette accent border by default;
`statefulChrome=false` is the explicit opt-out. `Controls.combo` is wholly framework-drawn and
...
| `Viewport` | `Viewport.resolve`, `Viewport.safe`, `Viewport.clamp`, `Viewport.clampAccessible` | Pure player/environment rectangle resolution; no lifecycle owner. `clampAccessible` preserves a small reachable header grip while allowing deliberate near-complete horizontal off-screen placement. |
| `Metrics` | `Metrics.tokens`, `Metrics.profile`, `Metrics.blockRects`, `Metrics.gridColumns`, `Metrics.inherit` | Returns copied token/profile/rectangle data. See the profile namespace table above. `inherit` resolves explicit overrides over parent/default metrics. |
...
| `Theme` | `Theme.tokens`, `Theme.set`, `Theme.color`, `Theme.palette`, `Theme.normalizeColor`, `Theme.context`, `Theme.resolveMaterial`, `Theme.apply`, `Theme.bind` | Theme queries return copied values; `normalizeColor` accepts a supported color form and returns a defensive RGBA value or fallback; `set` changes framework-local defaults; `apply` styles a caller-owned widget and does not own it. Context/material resolution and binding inherit palette state without transferring ownership. |
...
| `FocusStack` | `FocusStack.push`, `FocusStack.top`, `FocusStack.isTop`, `FocusStack.activate`, `FocusStack.activeWindow`, `FocusStack.handleEscape`, `FocusStack.remove`, `FocusStack.install`, `FocusStack.clearPlayer` | `push`/`install` return disposable per-player layers; removal/clear are idempotent cleanup operations. The latest opened or activated visible layer owns Escape for its player. Installed Escape pulses remain consumed through key release and close on a self-cleaning next-tick barrier, preventing the same pulse from reaching vanilla pause handling. |
...
|---|---|---|
| `Window` | `Window.derive`, `Window.callBase`, `Window.profile`, `Window.resolveBounds`, `Window.resolveInitialPosition`, `Window.updateConstraints`, `Window.safeRect`, `Window.clampBounds`, `Window.hasOverlap`, `Window.resizeHandleRect`, `Window.hitTestResizeHandle`, `Window.forgetGeometry`, `Window.chromeRects`, `Window.composeHeaderTitle`, `Window.render`, `Window.reflow`, `Window.apply`, `Window.create`, `Window.newInstance`, `Window.applyEditor` | `create`/`apply` are canonical. Geometry and title-composition helpers return values only. `resolveInitialPosition` applies the shared displaced-window policy while keeping a 32 px header recovery grip accessible at horizontal edges. `render` and static `reflow` are advanced hooks for an applied Window; they do not create a second owner. |
| `Modal` | `Modal.top`, `Modal.topForOwner`, `Modal.setOwner`, `Modal.raiseOwned`, `Modal.raiseOwner`, `Modal.resolve`, `Modal.apply`, `Modal.create`, `Modal.show`, `Modal.presentChild`, `Modal.close`, `Modal.fitContent`, `Modal.confirm`, `Modal.input`, `Modal.compact`, `Modal.task` | `top` lookups are read-only; `resolve` returns bounds/profile data; `create`/conveniences return Window-backed owned panels. `presentChild` binds lifecycle ownership without replacing native owner methods or installing a permanent pointer blocker; explicit activation preserves owner-to-child stacking. Modals resize only by explicit opt-in. |
...
| `Card` | `Card.metrics`, `Card.summaryMetrics`, `Card.measureData`, `Card.create` | `create` returns atomic final content with named icon/title/value/description/status slots and no recursive child host. `measureData` is the shared pure measurement path. The Card owns its panel; `setData`, `reflow` and `dispose` are instance operations. |
| `CardCollection` | `CardCollection.measure`, `CardCollection.create` | Returns a collection owning the cards it builds; measurement is pure and `setItems`, `reflow`, `clear` and `dispose` are instance operations. |
| `Collection` | `Collection.measure`, `Collection.create` | Returns a generic repeated-content owner; measurement is pure and item semantics remain consumer data. |
...
| `Controls` | `Controls.metrics`, `Controls.alignOffset`, `Controls.textPosition`, `Controls.truncateText`, `Controls.wrapText`, `Controls.renderWrappedLinePool`, `Controls.styleField`, `Controls.styleCombo`, `Controls.styleButton`, `Controls.panel`, `Controls.separator`, `Controls.button`, `Controls.fitButtonToContent`, `Controls.measureButtonWidth`, `Controls.iconButton`, `Controls.icon`, `Controls.field`, `Controls.combo`, `Controls.search`, `Controls.effectiveSearchQuery`, `Controls.toggle`, `Controls.listOption`, `Controls.measureStatus`, `Controls.status`, `Controls.setDanger`, `Controls.feedback`, `Controls.progress`, `Controls.alertRow`, `Controls.headerOperation`, `Controls.copyText`, `Controls.sectionTitle`, `Controls.blockHeader`, `Controls.requirementRow`, `Controls.dismissibleRow`, `Controls.dismissibleRowHeight`, `Controls.setTooltip`, `Controls.create` | `create` and named constructors return caller-owned controls or `nil, reason`; common callbacks use neutral context. `setTooltip` owns and reuses the framework attachment even when a concrete PZ control has no native tooltip setter. Alignment/metrics/text/measurement helpers are pure. `setDanger` applies the shared muted-danger style. |
...
| `Tooltip` | `Tooltip.measureSection`, `Tooltip.renderFrame`, `Tooltip.renderSection`, `Tooltip.appendSection`, `Tooltip.clearSections`, `Tooltip.setSections`, `Tooltip.measureSections`, `Tooltip.renderSections`, `Tooltip.createDocument`, `Tooltip.createScrollableSections`, `Tooltip.objectSection`, `Tooltip.makePassive`, `Tooltip.pointerPosition`, `Tooltip.position`, `Tooltip.hide`, `Tooltip.anchorToPointer`, `Tooltip.attach` | Measure/position helpers return values only; render helpers paint a supplied panel. Documents, scrollable sections, object sections and attachments remain separate framework owners around caller data. `attach(..., { variant = "transient" })` adds one-active-per-player/channel ownership, viewport clamping and optional active-only timeout. |
